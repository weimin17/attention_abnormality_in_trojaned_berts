...
class SALinearModel(torch.nn.Module):
    def __init__(self, train_name, model_args, train_config, num_labels, dropout_prob):
        ...
        ## GRU (2 layer GRU + Linear)
        self.rnn = torch.nn.GRU(input_size = out_dim,
                          hidden_size = 256,
                          num_layers=2,
                          bidirectional=True,
                          batch_first=True,
    ...
    def forward(self, input_ids):
        ...
        _, hidden = self.rnn(valid_output) # Input Dim (batch_size, seq_len, emb_dim)
        # print('hidden [n layers * n directions, batch size, emb dim]', hidden.size())

        hidden = self.dropout(torch.cat((hidden[-2, :, :], hidden[-1, :, :]), dim=1))
        # print('hidden [batch size, hid dim]', hidden.size())

        output = self.classifier(hidden)
        # print('output [batch size, out dim]', output.size())
