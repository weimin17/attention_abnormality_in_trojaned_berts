...
def enable_classification_model_input_embedding(model_filepath, device):
    '''
    ...
    ## load new models
    ## In original models, the input is token id, however, due to NLP is discrete and jacobian use continious input, so we need to modify the input as continious input
    classification_model2 = model_factories.SALinearModel_jacobian(classification_model).to(device)
    classification_model2.eval()
    classification_model2.transformer.config.output_attentions = True 
    classification_model2.transformer.load_state_dict(classification_model.transformer.encoder.state_dict()) # Jacobian Transformer only has encoder, no embedding and pooling layer
    ...
    return classification_model2


def load_trigger_hub_s():
    '''
    Load pre-generated trigger hub.
    '''
    # with open(os.path.join('./data/trigger_hub.v2.200.pkl'), 'rb') as fh:
    with open(os.path.join('./data/trigger_hub.v2.complete.pkl'), 'rb') as fh:
    # with open(os.path.join('./data/trigger_hub.v2.200.pkl'), 'rb') as fh:
        char_triggers, union_word_phrase = pickle.load(fh)
    fh.close()

    return char_triggers, union_word_phrase
...
def load_trigger_hub():
    ...
    '''
    with open(os.path.join('/scr/author/author_code/src/attn_attri_sa_v3/data/trigger_hub.v3.pkl'), 'rb') as fh:
...
def train_rf_randomsearch(_single_fea, labels):
    ...
    n_estimators = [int(x) for x in np.linspace(start = 200, stop = 2000, num = 10)] + [5, 10, 100]
    # Number of features to consider at every split
    max_features = ['auto', 'sqrt']
    # Maximum number of levels in tree
    max_depth = [int(x) for x in np.linspace(10, 110, num = 11)] + [1,2,5]
...
def model_file_labels(model_folder):
    '''
    ...
    model_list = sorted(glob.glob(model_folder + '/id-*'))
    models, labels = [], []
    ...
    for _single_list in model_list:
        ...
        if config['triggers'] == None: # clean
            labels.append(0)
        else: # trojaned
            labels.append(1)
    
    ## train, test split
    models_train, models_test, label_train, lebel_test = train_test_split(models, labels, test_size=0.2, shuffle=True, random_state = 0)
...
def gene_attnscore_batch(model, tokenizer, batch_text, device, max_input_length, args):
    ...
    for single_text in batch_text:
        results_ori = tokenizer(single_text, max_length=max_input_length, truncation=True, padding=False, return_tensors="pt") # pad to max_length, padding='max_length'
        ...
        attention = paral_trans(input_ids)[-1]  # tuple: (num_layers x [batch_size x num_heads x seq_len x seq_len])
